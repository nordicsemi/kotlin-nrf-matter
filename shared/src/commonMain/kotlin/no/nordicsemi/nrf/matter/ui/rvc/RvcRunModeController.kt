...
data class RvcRunModeData(
    val supportedModes: List<ModeOption>,
    val currentMode: Int,
)

class RvcRunModeController(
    private val cluster: RvcRunModeCluster,
    scope: CoroutineScope,
) : ClusterController(scope) {

    private val _state = MutableStateFlow<UiState<RvcRunModeData>>(UiState.Loading())
    ...
    init {
        ...
        combine(_supportedModes, cluster.observeCurrentMode()) { supportedModes, currentMode ->
            RvcRunModeData(supportedModes, currentMode.toInt())
        }

...
class KeypairHelper {
    ...
    func generatePrivateKey() throws -> SecKey {
        SwiftLogger.debug("\(self.logTag) - Generating new key.")
        ...
        if error != nil {
            SwiftLogger.debug("\(self.logTag) - Error during generation of a new key.")
        ...
        SwiftLogger.debug("\(self.logTag) - Returning newly generated key.")
    ...
    func getPrivateKey() -> SecKey? {
        SwiftLogger.debug("\(self.logTag) - Getting private key.")
        ...
        guard status == errSecSuccess else {
            SwiftLogger.debug("\(self.logTag) - Private key not found.")
            return nil
        }
        SwiftLogger.debug("\(self.logTag) - Private key found.")
        guard item != nil else {
            SwiftLogger.debug("\(self.logTag) - Private key is nil. Deleting.")
            deletePrivateKey()
            return nil
        }
    ...
    func deletePrivateKey() {
        SwiftLogger.debug("\(self.logTag) - Deleting private key.")
